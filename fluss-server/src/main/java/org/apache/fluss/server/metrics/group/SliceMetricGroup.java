...
/** Metrics for slice table management. */
public class SliceMetricGroup extends AbstractMetricGroup {

    private static final String NAME = "slice";

    private final Map<Long, TableMetricGroup> metricGroupByTable = new HashMap<>();

    public SliceMetricGroup(MetricRegistry registry, CoordinatorMetricGroup parent) {
    ...
    //  table slice groups
    // ------------------------------------------------------------------------
    public MetricGroup addTableSliceMetricGroup(long tableId, TablePath tablePath) {
        return metricGroupByTable.computeIfAbsent(
                tableId, table -> new TableMetricGroup(registry, this, tablePath, tableId));
    }

    public void removeTableSliceMetricGroup(long tableId) {
